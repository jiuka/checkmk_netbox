...
def _parameter_form_netbox_data_source():
    return Dictionary(
        elements={
            'maxage': DictElement(
                parameter_form=SimpleLevels(
                    title=Title('Maximal time since last sync'),
...
rule_spec_netbox_data_source = CheckParameters(
    name='netbox_data_source',
    topic=Topic.APPLICATIONS,
    parameter_form=_parameter_form_netbox_data_source,
    title=Title('Netbox DataSource'),
    help_text=Help('This rule configures thresholds Netbox DataSource.'),
    condition=HostAndItemCondition(item_title=Title('Netbox DataSource')),
)
